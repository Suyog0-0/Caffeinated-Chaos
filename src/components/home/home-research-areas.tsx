    ...
    <section className={`${pageShell} py-24 max-sm:py-16`}>
      <header className="mb-12 grid grid-cols-[1.2fr_.8fr] items-end gap-16 max-sm:grid-cols-1 max-sm:gap-5">
        <div>
          ...
          <h2 className={sectionTitle}>Explore research areas</h2>
        </div>
        <p className="max-w-lg text-[#405149]">
          Follow a subject to the researchers investigating it, the projects in progress, and the
          evidence they publish.
        </p>
      </header>
      <div className="border-t border-[#17251f]">
        {researchAreas.map((area, index) => (
          ...
          >
            ...
            <span>
              <strong className="block text-3xl font-medium max-sm:text-2xl">{area.title}</strong>
            ...
            <span className="flex gap-7 font-sans text-[10px] text-[#405149] max-sm:col-start-2">
              <small>{area.projects} projects</small>
              <small>{area.papers} publications</small>
            </span>
      ...
      <Link
        className={buttonVariants({ variant: "link", className: "mt-8" })}
        href="/research-areas"
      >
        Browse all research areas <ChevronRight aria-hidden size={16} />
      </Link>
    </section>
  );
}
