...
import { deleteEthicsPolicyAction } from "@/app/ethics-policy-actions";

export function DeleteEthicsPolicyButton({ id, title }: { id: string; title: string }) {
  const [confirming, setConfirming] = useState(false);
  const action = deleteEthicsPolicyAction.bind(null, id);

  if (!confirming) {
    return <button aria-label={`Delete ${title}`} className="inline-flex size-9 items-center justify-center border border-[#d4d5ce] bg-[#fffefb] text-[#53645c] hover:border-[#8f3939] hover:text-[#8f3939] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#8f3939]" onClick={() => setConfirming(true)} title="Delete ethics policy" type="button"><Trash2 size={17} /></button>;
  }
