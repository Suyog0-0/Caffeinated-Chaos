  ...
  return (
    <section className="bg-[#0d2a20] py-20 text-white">
      <div className="mx-auto w-[min(calc(100%_-_48px),1240px)] max-sm:w-[calc(100%_-_32px)]">
        <p className="mb-4 font-sans text-lg font-bold uppercase">Leadership</p>
        <h2 className="text-[42px] leading-tight font-medium">
          Guided by researchers and educators.
        </h2>
        <div className="mt-10 grid grid-cols-3 border-t border-[#536b61] max-sm:grid-cols-1">
          {leaders.map(([initials, name, role]) => (
            ...
            >
              <CardHeader className="px-0">
                <span className="grid size-18 place-items-center rounded-full border border-[#799087] font-sans text-[20px]">
                  {initials}
                </span>
                <CardTitle className="mt-3 text-[28px] font-medium">{name}</CardTitle>
              </CardHeader>
              <CardContent className="px-0 text-xl text-[#b7c6be]">{role}</CardContent>
            </Card>
          ))}
        </div>
      </div>
