...
export function GrantList({ grants }: { grants: Grant[] }) {
    ...
    const filtered = grants.filter((g) => {
        ...
        if (query) {
            const haystack = `${g.title} ${g.funder ?? ""} ${
                g.description ?? ""
            } ${g.eligibility ?? ""}`.toLowerCase();
    ...
    return (
        <div className="pb-16">
            {sorted.map((grant, i) => {
                ...
                const statusStyle =
                    STATUS_STYLES[grant.status] ?? STATUS_STYLES.closed;

                return (
                    <details
                        key={grant.id}
                        className={`group ${
                            i === 0 ? "" : "border-t border-[#e5dfd3]"
                        }`}
                    >
                        <summary className="grid cursor-pointer grid-cols-1 list-none gap-4 py-8 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0e2820]/30 lg:grid-cols-12 lg:gap-8 lg:items-start [&::-webkit-details-marker]:hidden">
                            ...
                            <div className="lg:col-span-2">
                                ...
                                <p className="mt-1 font-serif text-4xl leading-none text-[#141d18]">
                                    {deadline.day}
                                </p>
                            </div>
                            ...
                            <div className="space-y-2 lg:col-span-7">
                                <span
                                    className={`inline-flex items-center rounded px-2.5 py-0.5 text-xs font-medium ${statusStyle}`}
                                >
                            ...
                            <div className="flex items-center justify-between gap-3 lg:col-span-3 lg:flex-col lg:items-end lg:justify-center">
                                {amount ? (
                                    <p className="font-serif text-lg text-[#0e2820]">
                                        {amount}
                                    </p>
                                ) : (
                                    <span />
