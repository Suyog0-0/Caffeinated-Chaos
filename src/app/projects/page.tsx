...
export default async function ProjectsPage() {
  ...
  const { data } = await supabase
    .from("project")
    .select(
      "slug, status, title, description, research_area(name, slug), project_researcher(role, researcher(name))"
    )
  ...
  const projects = (data ?? []).map((p) => ({
    ...
    status: p.status,
    areaSlug:
      (p.research_area as unknown as { slug: string } | null)?.slug ?? "",
    area:
    ...
    lead:
      (
        p.project_researcher as unknown as {
          role: string;
