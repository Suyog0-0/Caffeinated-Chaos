...
export default async function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
        <main className={`${inter.className} bg-[#FBF9F5] pb-24`}>
            <EventDetailHero
                title={event.title}
            ...
            <div className="mx-auto grid w-[min(calc(100%_-_48px),1240px)] grid-cols-[1fr_340px] gap-[6vw] pt-14 max-lg:grid-cols-1 max-lg:gap-10 max-sm:w-[calc(100%_-_32px)]">
                <article>
                    <h2 className="mb-6 font-serif text-4xl font-medium tracking-tight text-[#0F2D24]">About this event</h2>
                    <div className="prose max-w-none">
                        <p className="whitespace-pre-wrap text-justify text-[17px] leading-8 text-[#293530]">
